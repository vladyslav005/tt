...
import type {NilNodeData} from "@/shared/presentation/flow/types.ts";
import {cn} from "@/shared/lib/utils";
import {LimitedHandle} from "./LimitedHandle.tsx";

export function NilFlowNode({selected}: { data: NilNodeData; selected?: boolean }) {
  return (
    <div className={cn(
      ...
      <div className="px-4 pt-4 pb-3 flex items-center gap-2 border-b border-sky-500/15">
        <span className="text-sm font-bold text-sky-600 dark:text-sky-400">nil</span>
        <span className="text-xs font-semibold text-sky-600 dark:text-sky-400">Empty List</span>
      </div>
