...
import type {IsNilNodeData} from "@/shared/presentation/flow/types.ts";
import {cn} from "@/shared/lib/utils";
import {LimitedHandle} from "./LimitedHandle.tsx";

export function IsNilFlowNode({selected}: { data: IsNilNodeData; selected?: boolean }) {
  return (
    <div className={cn(
      ...
      <div className="px-4 pt-4 pb-3 flex items-center gap-2 border-b border-sky-500/15">
        <span className="text-sm font-bold text-sky-600 dark:text-sky-400">isnil</span>
        <span className="text-xs font-semibold text-sky-600 dark:text-sky-400">Is Empty?</span>
      </div>
