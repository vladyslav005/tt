...
import type {HeadNodeData} from "@/shared/presentation/flow/types.ts";
import {cn} from "@/shared/lib/utils";
import {LimitedHandle} from "./LimitedHandle.tsx";

export function HeadFlowNode({selected}: { data: HeadNodeData; selected?: boolean }) {
  return (
    <div className={cn(
      ...
      <div className="px-4 pt-4 pb-3 flex items-center gap-2 border-b border-sky-500/15">
        <span className="text-sm font-bold text-sky-600 dark:text-sky-400">head</span>
        <span className="text-xs font-semibold text-sky-600 dark:text-sky-400">First Element</span>
      </div>
      ...
      <div className="p-4 grid grid-cols-2 gap-3">
        <div className="relative flex flex-col items-center gap-1 rounded-lg border border-sky-200 dark:border-sky-800 bg-background/50 px-3 py-3">
          <span className="text-xs font-medium text-sky-600 dark:text-sky-400">list</span>
