#!/bin/bash
# Bumps packages/core's version, then commits + tags it directly. Does NOT use `npm version`'s
...
npm version "$BUMP" --no-git-tag-version --workspace=@vladyslav005/tt-core

NEW_VERSION="$(node -p "require('./packages/core/package.json').version")"
TAG="core-v${NEW_VERSION}"
